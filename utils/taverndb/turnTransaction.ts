...
export interface TurnTransactionSourceOwnershipRules {
...
export interface TurnTransactionOptions {
  ...
  sourceOwnershipRules?: TurnTransactionSourceOwnershipRules;
}
...
type StateRollbackReason = 'source_not_allowed' | 'row_version_conflict' | 'sheet_version_conflict' | 'apply_error';

const resolveRollbackReasonFromConflicts = (conflicts: TavernDBPatchConflict[]): StateRollbackReason => {
  if (!Array.isArray(conflicts) || conflicts.length === 0) return 'apply_error';
  if (conflicts.some((item) => item.reason === 'source_not_allowed')) return 'source_not_allowed';
  if (conflicts.some((item) => item.reason === 'row_version_conflict')) return 'row_version_conflict';
  if (conflicts.some((item) => item.reason === 'sheet_version_conflict')) return 'sheet_version_conflict';
  return 'apply_error';
};

const appendRollbackSummary = <TState>(
  state: TState,
  reason: StateRollbackReason,
  count: number = 1
): TState => {
  ...
  const outcome = (currentMeta as any).stateVarOutcome && typeof (currentMeta as any).stateVarOutcome === 'object'
    ? (currentMeta as any).stateVarOutcome
    : {};
  const rollback = outcome.rollback && typeof outcome.rollback === 'object'
    ? outcome.rollback
    : {};
  const byReason = rollback.byReason && typeof rollback.byReason === 'object'
    ? { ...rollback.byReason }
    : {};
  byReason[reason] = Number(byReason[reason] || 0) + Math.max(1, Math.floor(Number(count || 1)));
...
const isAllowedBySourcePrefixes = (rawSource: unknown, prefixes: string[]): boolean => {
  const source = String(rawSource || '').trim();
...
const resolveProtectedSheetSourceConflicts = (
  patches: TavernDBSheetPatch[],
  rules?: TurnTransactionSourceOwnershipRules
): TavernDBPatchConflict[] => {
  const protectedSheets = new Set(
    Array.isArray(rules?.protectedSheets)
      ? rules!.protectedSheets.map((item) => String(item || '').trim()).filter(Boolean)
      : []
  );
  if (protectedSheets.size === 0) return [];
  const allowedPrefixes = Array.isArray(rules?.allowedSourcePrefixes) && rules!.allowedSourcePrefixes.length > 0
    ? rules!.allowedSourcePrefixes
        .map((item) => String(item || '').trim())
        .filter(Boolean)
    : ['ms:state-writer'];

  return patches
    .filter((patch) => protectedSheets.has(String(patch.sheetId || '').trim()))
    .filter((patch) => !isAllowedBySourcePrefixes(patch.source, allowedPrefixes))
    .map((patch) => {
      const source = String(patch.source || '').trim() || 'unspecified';
      return {
        sheetId: patch.sheetId,
        rowId: patch.rowId,
        reason: 'source_not_allowed',
        message: `source=${source}; expected=${allowedPrefixes.join('|')}`
      } as TavernDBPatchConflict;
    });
};
...
const resolvePatchConflicts = <TState>(
  ...
  rollbackReason?: StateRollbackReason;
} => {
  ...
  const sourceOwnershipConflicts = [
    ...resolveSourceOwnershipConflicts(patches),
    ...resolveProtectedSheetSourceConflicts(patches, options?.sourceOwnershipRules)
  ];
  if (sourceOwnershipConflicts.length > 0) {
    ...
    return {
      ok: false,
      nextState: appendRollbackSummary(cloneStateWithMeta(baseState, nextMeta), 'source_not_allowed'),
      logs: [blockedLog],
      appliedPatchCount: 0,
      rollbackReason: 'source_not_allowed'
    };
  }
  ...
  return {
    ok: false,
    nextState: appendRollbackSummary(
      cloneStateWithMeta(baseState, store.exportMeta()),
      resolveRollbackReasonFromConflicts(report.conflicts),
      report.conflicts.length
    ),
    logs: [blockedLog],
    appliedPatchCount: 0,
    rollbackReason: resolveRollbackReasonFromConflicts(report.conflicts)
  };
};
...
export const applyTurnTransaction = <TState>(
  ...
  if (!shouldUseTransaction(commands, options)) {
    ...
    if (!hasApplyError(directResult) && directPatches.length > 0) {
      const conflictCheck = resolvePatchConflicts(baseState, directResult.newState, directPatches, txId, options);
      if (!conflictCheck.ok) {
        const tracedState = appendTransactionTrace(conflictCheck.nextState, {
          ...
          sources: txSources,
          reason: conflictCheck.rollbackReason || 'apply_error'
        });
        return {
  ...
  if (!hasApplyError(applied)) {
    const conflictCheck = resolvePatchConflicts(baseState, applied.newState, applied.sheetPatches || [], txId, options);
    if (!conflictCheck.ok) {
      const tracedState = appendTransactionTrace(conflictCheck.nextState, {
        txId,
        timestamp: Date.now(),
  ...
  return {
    newState: appendRollbackSummary(rollbackState, 'apply_error'),
