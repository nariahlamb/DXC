...
import type {
  StateVarDomainSheetFieldAllowlist,
  StateVarInvalidConfigFallbackStrategy
} from '../../types/ai';
...
const TEMPLATE_SHEET_REGISTRY: TavernDBSheetDefinition[] = [
  ...
  {
    id: 'SYS_StateVarEventLog',
    label: '变量事件日志',
    description: '记录 state variable bridge/writer 事件载荷与来源。',
    order: 5,
    columns: [
      { key: 'event_id', required: true },
      { key: 'turn_id', required: true },
      { key: 'source', required: true },
      { key: 'domain', required: true },
      { key: 'entity_id', required: true },
      { key: 'path', required: true },
      { key: 'op', required: true },
  ...
  {
    id: 'SYS_StateVarApplyLog',
    label: '变量应用日志',
    description: '记录 state variable writer 的应用结果、重试与冲突。',
    order: 6,
    columns: [
      { key: 'apply_id', required: true },
      { key: 'event_id', required: true },
      { key: 'tx_id' },
      { key: 'sheet_id', required: true },
      { key: 'row_id', required: true },
      { key: 'result', required: true },
      { key: 'conflict_reason' },
      { key: 'retry_count' },
      { key: 'latency_ms' },
      { key: 'applied_at', required: true }
    ]
  },
  {
    id: 'ECON_Ledger',
...
const DOMAIN_MAPPING_REGISTRY: TavernDBDomainMapping[] = [
  ...
  { domain: 'state_var_event_log', module: 'core', sheetId: 'SYS_StateVarEventLog', primaryKey: 'event_id', description: '变量事件日志' },
  { domain: 'state_var_apply_log', module: 'core', sheetId: 'SYS_StateVarApplyLog', primaryKey: 'apply_id', description: '变量应用日志' },
...
const cloneAllowlist = (
  source: StateVarDomainSheetFieldAllowlist
): StateVarDomainSheetFieldAllowlist => {
  const cloned: StateVarDomainSheetFieldAllowlist = {};
  Object.entries(source).forEach(([domain, sheetMap]) => {
    cloned[domain] = {};
    Object.entries(sheetMap).forEach(([sheetId, fields]) => {
      cloned[domain][sheetId] = Array.isArray(fields) ? [...fields] : [];
    });
  });
  return cloned;
};

export const buildDefaultStateVariableAllowlist = (): StateVarDomainSheetFieldAllowlist => {
  const columnsBySheet = new Map<string, string[]>(
    TEMPLATE_SHEET_REGISTRY.map((sheet) => [sheet.id, sheet.columns.map((column) => String(column.key))])
  );
  const allowlist: StateVarDomainSheetFieldAllowlist = {};
  DOMAIN_MAPPING_REGISTRY.forEach((mapping) => {
    const domain = String(mapping.domain || '').trim();
    const sheetId = String(mapping.sheetId || '').trim();
    if (!domain || !sheetId) return;
    const columns = columnsBySheet.get(sheetId);
    if (!columns || columns.length === 0) return;
    if (!allowlist[domain]) allowlist[domain] = {};
    allowlist[domain][sheetId] = [...columns];
  });
  return allowlist;
};

export interface NormalizeStateVariableAllowlistResult {
  allowlist: StateVarDomainSheetFieldAllowlist;
  usedFallback: boolean;
  hasInvalidEntries: boolean;
}

export const normalizeStateVariableAllowlist = (
  rawAllowlist: unknown,
  fallbackStrategy: StateVarInvalidConfigFallbackStrategy = 'use_default_allowlist'
): NormalizeStateVariableAllowlistResult => {
  const defaultAllowlist = buildDefaultStateVariableAllowlist();
  const validDomains = new Set<string>(Object.keys(defaultAllowlist));
  const hasRawObject = !!rawAllowlist && typeof rawAllowlist === 'object' && !Array.isArray(rawAllowlist);
  if (!hasRawObject) {
    return {
      allowlist: defaultAllowlist,
      usedFallback: true,
      hasInvalidEntries: rawAllowlist !== undefined && rawAllowlist !== null
    };
  }

  const normalizedCustom: StateVarDomainSheetFieldAllowlist = {};
  let hasInvalidEntries = false;

  Object.entries(rawAllowlist as Record<string, unknown>).forEach(([rawDomain, rawSheetMap]) => {
    const domain = String(rawDomain || '').trim();
    if (!domain || !validDomains.has(domain)) {
      hasInvalidEntries = true;
      return;
    }
    if (!rawSheetMap || typeof rawSheetMap !== 'object' || Array.isArray(rawSheetMap)) {
      hasInvalidEntries = true;
      return;
    }

    const allowedSheetMap = defaultAllowlist[domain];
    const validSheets = new Set<string>(Object.keys(allowedSheetMap));
    const nextSheetMap: Record<string, string[]> = {};
    Object.entries(rawSheetMap as Record<string, unknown>).forEach(([rawSheetId, rawFields]) => {
      const sheetId = String(rawSheetId || '').trim();
      if (!sheetId || !validSheets.has(sheetId)) {
        hasInvalidEntries = true;
        return;
      }
      ...
      const fieldSet = new Set<string>(allowedSheetMap[sheetId] || []);
      ...
      nextSheetMap[sheetId] = fields;
    });

    if (Object.keys(nextSheetMap).length > 0) {
      normalizedCustom[domain] = nextSheetMap;
    } else {
      hasInvalidEntries = true;
    }
  });

  const hasValidCustomEntries = Object.keys(normalizedCustom).length > 0;
  if (!hasValidCustomEntries) {
    return {
      allowlist: defaultAllowlist,
      usedFallback: true,
      hasInvalidEntries: true
  ...
  if (hasInvalidEntries && fallbackStrategy === 'use_default_allowlist') {
    return {
      allowlist: defaultAllowlist,
      usedFallback: true,
      hasInvalidEntries: true
  ...
  if (hasInvalidEntries && fallbackStrategy === 'merge_with_default_allowlist') {
    const merged = cloneAllowlist(defaultAllowlist);
    Object.entries(normalizedCustom).forEach(([domain, sheetMap]) => {
      if (!merged[domain]) merged[domain] = {};
      Object.entries(sheetMap).forEach(([sheetId, fields]) => {
        merged[domain][sheetId] = [...fields];
      });
    });
    return {
      allowlist: merged,
  ...
  return {
    allowlist: cloneAllowlist(normalizedCustom),
    usedFallback: false,
