...
describe('sheet registry', () => {
  it('loads template sheets with full registry', () => {
    ...
    expect(TAVERNDB_TEMPLATE_SHEET_IDS).toHaveLength(54);
  ...
  it('exposes domain mapping registry as readable ssot', () => {
    ...
    expect(mappings.some((mapping) => mapping.domain === 'state_var_event_log' && mapping.sheetId === 'SYS_StateVarEventLog')).toBe(true);
    expect(mappings.some((mapping) => mapping.domain === 'state_var_apply_log' && mapping.sheetId === 'SYS_StateVarApplyLog')).toBe(true);
    ...
    expect(mappings.some((mapping) => mapping.domain === 'global_state' && mapping.primaryKey === '_global_id')).toBe(true);
  });
});
