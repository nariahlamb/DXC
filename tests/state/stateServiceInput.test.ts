...
const createGovernanceContract = () => ({
  ...
  turnScope: {
    crossTurnSoftWarning: {
      ...
      sampling: 1,
      nonBlocking: true,
      escalation: { enabled: false, threshold: 3, action: 'warn' as const }
    }
  },
  domainScope: {
...
describe('buildStateServiceInputPayload', () => {
  ...
  it('keeps sheet guide and fill-task metadata', () => {
    ...
    expect(payload.治理契约.version).toBe('state-variable-governance-v1');
    expect(payload.治理契约.turnScope.crossTurnSoftWarning.nonBlocking).toBe(true);
    expect(payload.治理契约.domainScope.strictAllowlist).toBe(true);
    expect(payload.治理契约.semanticScope.anchors).toEqual(['economy']);
  ...
  it('annotates out-of-scope requiredSheets in governance contract domainScope context', () => {
    const state = createNewGameState('Tester', '男', 'Human') as any;

    const requiredSheets = ['SYS_GlobalState', 'WORLD_News'];
    const payload = buildStateServiceInputPayload({
      state,
      socialBrief: [],
      requiredSheets,
      stateSheetGuide: [],
      maxConcurrentSheets: 4,
      maxConcurrentBatches: 2,
      governanceContract: createGovernanceContract()
    }) as any;

    const ctx = payload.治理契约?.domainScope?.context;
    expect(ctx?.requiredSheets).toEqual(requiredSheets);
    expect(ctx?.requiredSheetCount).toBe(2);
    expect(ctx?.outOfScopeRequiredSheets).toEqual(['WORLD_News']);
    expect(ctx?.hasOutOfScopeRequiredSheets).toBe(true);
  });
});
