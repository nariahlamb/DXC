...
export function handleUpsertNPC(
    ...
    const normalizeNpcRow = (row: Record<string, unknown>): Record<string, unknown> | null => {
        ...
        const hasAffinityLiteral = affinityRaw !== undefined
            && affinityRaw !== null
            && !(typeof affinityRaw === 'string' && affinityRaw.trim().length === 0);
        const parsedAffinity = hasAffinityLiteral ? toNumber(affinityRaw) : null;
    ...
    newNPCs.forEach(npc => {
        ...
        const safeNpcPatch = Object.fromEntries(
            Object.entries(safeNpc as Record<string, unknown>).filter(([, value]) => value !== undefined)
...
export function handleUpsertInventory(
    ...
    const normalizeInventoryRow = (row: Record<string, unknown>): Record<string, unknown> | null => {
        ...
        const quality = toText(row.品质 ?? row.稀有度 ?? row.quality ?? row.rarity ?? row.rank ?? row.tier);
        const normalizedQuality = quality ? normalizeQualityLabel(quality) : undefined;
        return {
            ...row,
    ...
    newItems.forEach(item => {
        const definedItem = Object.fromEntries(
            Object.entries(item as Record<string, unknown>).filter(([, value]) => value !== undefined)
        ) as Record<string, unknown>;
        const itemQuality = toText((definedItem as any).品质 ?? (definedItem as any).稀有度);
        if (itemQuality) {
            const normalizedQuality = normalizeQualityLabel(itemQuality);
            (definedItem as any).品质 = normalizedQuality;
            (definedItem as any).稀有度 = normalizedQuality;
        }
        let index = -1;
        // A-007 FIX: Match by both 名称 AND 品质 to prevent item loss when qualities differ
        if ((definedItem as any).id) {
            index = state.背包.findIndex(existing => existing.id === (definedItem as any).id);
        } else {
            const incomingQuality = toText((definedItem as any).品质 ?? (definedItem as any).稀有度);
            index = state.背包.findIndex(existing =>
                incomingQuality
                    ? (existing.名称 === (definedItem as any).名称 && existing.品质 === incomingQuality)
                    : (existing.名称 === (definedItem as any).名称)
            );
        }

        if (index >= 0) {
            state.背包[index] = { ...state.背包[index], ...definedItem };
        } else {
            const newItem = {
                id: (definedItem as any).id || crypto.randomUUID(),
                数量: 1,
                描述: '',
                类型: '杂项',
                ...definedItem
            };
            if (!toText((newItem as any).品质 ?? (newItem as any).稀有度)) {
                (newItem as any).品质 = '普通';
                (newItem as any).稀有度 = '普通';
            }
...
const upsertByKey = <T extends Record<string, any>>(list: T[], rows: T[], keyField: string) => {
    rows.forEach((row) => {
        ...
        const key = sanitizedRow?.[keyField];
        if (key === undefined || key === null || key === '') {
            list.push(sanitizedRow);
            return;
        }
        const index = list.findIndex((item) => item?.[keyField] === key);
        if (index >= 0) {
            list[index] = { ...list[index], ...sanitizedRow };
        } else {
            list.push(sanitizedRow);
        }
    });
...
const upsertRowsInShadowStore = (
    ...
    rows.forEach((row) => {
        ...
        if (idx >= 0) {
            list[idx] = { ...list[idx], ...sanitizedRow };
        } else {
            list.push({ ...sanitizedRow });
        }
    });
...
const normalizeNpcSheetRows = (rows: Array<Record<string, unknown>>) => {
    return rows
        .map((row) => {
            ...
            const safeRowPatch = Object.fromEntries(
                Object.entries(safeRow).filter(([, value]) => value !== undefined)
            ) as Record<string, unknown>;
            return {
                ...safeRowPatch,
                id,
...
const normalizeEconomicLedgerSheetRows = (rows: Array<Record<string, unknown>>, state: GameState) => {
    return rows
        .map((row, index) => {
            const account = toText(pickRowValue(row, ['account', '账户', '账号', 'account_id']));
            const normalizedAccount = account === '眷族.资金' ? '眷族.资金' : '角色.法利';
            const current = readEconomicAccount(state, normalizedAccount) ?? 0;
            const beforeRaw = toNumber(pickRowValue(row, ['before', '变更前', 'balance_before']));
            const deltaRaw = toNumber(pickRowValue(row, ['delta', '增量', '变化']));
            const afterRaw = toNumber(pickRowValue(row, ['after', '变更后', 'balance_after']));
            const before = beforeRaw ?? current;
            const delta = deltaRaw ?? (afterRaw !== null ? (afterRaw - before) : 0);
            const after = afterRaw ?? (before + delta);
            const reason = toText(pickRowValue(row, ['reason', '原因', '备注'])) || 'sheet_upsert';
            return {
...
const applyEconomicLedgerRowsToAccounts = (
    state: GameState,
    rows: Array<Record<string, unknown>>
): { success: boolean; error?: string } => {
    for (const row of rows) {
        const account = toText((row as any)?.account) === '眷族.资金' ? '眷族.资金' : '角色.法利';
        const current = readEconomicAccount(state, account);
        if (current === null) {
            return { success: false, error: `无法读取经济账户: ${account}` };
        }
        const after = toNumber((row as any)?.after);
        const delta = toNumber((row as any)?.delta);
        const nextValue = after ?? (delta !== null ? (current + delta) : null);
        if (nextValue === null) continue;
        if (nextValue < 0) {
            return { success: false, error: `经济守卫阻断: ${account} 变更后为负数 (${nextValue})` };
        }
        if (!updateEconomicAccount(state, account, nextValue)) {
            return { success: false, error: `无法写入经济账户: ${account}` };
        }
    }
    return { success: true };
};
...
const upsertFactionFromSheetRows = (state: GameState, rows: Array<Record<string, unknown>>) => {
    ...
    rows.forEach((row, index) => {
        ...
        const relationRaw = toText(row.关系等级 ?? row.关系);
        const reputation = toNumber(row.声望值 ?? row.声望);
        const description = toText(row.关键事件);
        const nextFaction = Object.fromEntries(
            Object.entries({
            id,
            名称: name,
            类型: '势力' as const,
            声望: reputation !== null ? reputation : undefined,
            关系: relationRaw ? toRelation(relationRaw) : undefined,
            描述: description || undefined
            } as Record<string, unknown>).filter(([, value]) => value !== undefined)
        );
        ...
        if (idx >= 0) {
            ...
            state.势力!.push({
                ...
                类型: '势力',
                声望: reputation ?? 0,
                关系: relationRaw ? toRelation(relationRaw) : '未知',
                描述: description || undefined
            } as any);
        }
    });
...
const normalizeFamiliaName = (value: unknown): string => {
    const text = toText(value);
    if (!text) return '';
    const normalized = text.trim();
    const lowered = normalized.toLowerCase();
    if (['none', 'null', 'nil', 'n/a', 'na'].includes(lowered)) return '无';
    return normalized;
};

const normalizeSkillCategory = (value: unknown): string => {
    const text = toText(value);
    if (!text) return '主动';
    const lowered = text.toLowerCase();
    if (lowered.includes('passive') || text.includes('被动')) return '被动';
    if (lowered.includes('support') || text.includes('支援')) return '支援';
    if (lowered.includes('special') || text.includes('特化') || text.includes('特殊')) return '特化';
    return text;
};

const isMagicSkillLibraryRow = (row: Record<string, unknown>): boolean => {
    const typeText = toText(pickRowValue(row, ['技能类型', 'skillType', 'type', '类别'])).toLowerCase();
    if (/(法术|魔法|spell|magic|咒)/.test(typeText)) return true;
    const spellTier = toText(pickRowValue(row, ['环阶', 'ring', 'tier', 'spell_level']));
    if (spellTier) return true;
    const school = toText(pickRowValue(row, ['学派', 'school']));
    if (school) return true;
    return false;
};

const normalizeSkillLibraryRows = (rows: Array<Record<string, unknown>>) => {
    const normalized = rows
        .map((row, index) => {
            const skillId = toText(pickRowValue(row, ['SKILL_ID', 'skill_id', 'id'])) || `SKILL_${index + 1}`;
            const skillName = toText(
                pickRowValue(row, ['技能名称', 'skill_name', 'name', '名称', 'title'])
            );
            ...
            return {
                ...
                SKILL_ID: skillId,
                技能名称: skillName,
                技能类型: toText(pickRowValue(row, ['技能类型', 'skillType', 'type', '类别'])) || '主动',
                环阶: toText(pickRowValue(row, ['环阶', 'ring', 'tier', 'spell_level'])) || '',
                学派: toText(pickRowValue(row, ['学派', 'school', '属性'])) || '',
                施法时间: toText(pickRowValue(row, ['施法时间', 'casting_time', 'castTime', '咏唱', '触发'])) || '',
                射程: toText(pickRowValue(row, ['射程', 'range', '范围'])) || '',
                持续时间: toText(pickRowValue(row, ['持续时间', 'duration', '持续'])) || '',
                效果描述: toText(
                    pickRowValue(row, ['效果描述', 'effect', 'description', '效果', '描述'])
                ) || '',
                升阶效果: toText(pickRowValue(row, ['升阶效果', 'upcast', '备注'])) || ''
            } as Record<string, unknown>;
        })
        .filter((row): row is Record<string, unknown> => !!row);
    ...
    normalized.forEach((row) => {
        const skillId = toText(row.SKILL_ID);
        if (!skillId) return;
        deduped.set(skillId, row);
    });
    return Array.from(deduped.values());
};

const normalizeCharacterSkillLinkRows = (rows: Array<Record<string, unknown>>) => {
    const normalized = rows
        .map((row, index) => {
            const rawCharId = toText(pickRowValue(row, ['CHAR_ID', 'char_id', 'character_id']));
            if (rawCharId && rawCharId !== 'PC_MAIN') return null;
            const skillId = toText(pickRowValue(row, ['SKILL_ID', 'skill_id']));
            if (!skillId) return null;
            const linkId = toText(pickRowValue(row, ['LINK_ID', 'link_id', 'id'])) || `SLINK_${index + 1}`;
            return {
                ...row,
                LINK_ID: linkId,
                CHAR_ID: 'PC_MAIN',
                SKILL_ID: skillId,
                获取方式: toText(pickRowValue(row, ['获取方式', 'source'])) || '',
                已准备: toText(pickRowValue(row, ['已准备', 'prepared'])) || '',
                熟练度: toText(pickRowValue(row, ['熟练度', 'proficiency'])) || '',
                备注: toText(pickRowValue(row, ['备注', 'note'])) || ''
            } as Record<string, unknown>;
        })
        .filter((row): row is Record<string, unknown> => !!row);
    if (normalized.length === 0) return normalized;
    ...
    normalized.forEach((row) => {
        ...
        if (!linkId) return;
        deduped.set(linkId, row);
    });
    return Array.from(deduped.values());
};

const rebuildCharacterSkillsFromShadowStore = (state: GameState) => {
    if (!state.角色) return;
    const store = ensureSheetShadowStore(state);
    const libraryRows = normalizeSkillLibraryRows(
        Array.isArray(store.SKILL_Library) ? store.SKILL_Library as Array<Record<string, unknown>> : []
    );
    if (libraryRows.length === 0) return;

    const linkRows = normalizeCharacterSkillLinkRows(
        Array.isArray(store.CHARACTER_Skills) ? store.CHARACTER_Skills as Array<Record<string, unknown>> : []
    );
    const linkedSkillIds = linkRows
        .map((row) => toText(row.SKILL_ID))
        .filter(Boolean);
    const sourceSkillIds = linkedSkillIds.length > 0
        ? linkedSkillIds
        : libraryRows.map((row) => toText(row.SKILL_ID)).filter(Boolean);
    const orderedSkillIds = Array.from(new Set(sourceSkillIds));
    const libraryById = new Map<string, Record<string, unknown>>();
    libraryRows.forEach((row) => {
        const skillId = toText(row.SKILL_ID);
        if (!skillId) return;
        libraryById.set(skillId, row);
    });

    const nextSkills: Array<Record<string, unknown>> = [];
    const nextMagic: Array<Record<string, unknown>> = [];
    orderedSkillIds.forEach((skillId, index) => {
        const row = libraryById.get(skillId);
        if (!row) return;
        const name = toText(pickRowValue(row, ['技能名称', 'skill_name', 'name', '名称']));
        if (!name) return;
        const skillType = toText(pickRowValue(row, ['技能类型', 'skillType', 'type', '类别'])) || '主动';
        const castingTime = toText(pickRowValue(row, ['施法时间', 'casting_time', 'castTime', '咏唱', '触发'])) || '';
        const range = toText(pickRowValue(row, ['射程', 'range', '范围'])) || '';
        const duration = toText(pickRowValue(row, ['持续时间', 'duration', '持续'])) || '';
        const effect = toText(pickRowValue(row, ['效果描述', 'effect', 'description', '效果', '描述'])) || '';
        const school = toText(pickRowValue(row, ['学派', 'school', '属性'])) || '';
        const tier = toText(pickRowValue(row, ['环阶', 'ring', 'tier', 'spell_level'])) || '';
        const note = toText(pickRowValue(row, ['升阶效果', 'upcast', '备注'])) || '';

        if (isMagicSkillLibraryRow(row)) {
            nextMagic.push({
                id: skillId || `MAG_${index + 1}`,
                名称: name,
                咏唱: castingTime || '无咏唱',
                类别: /(法术|魔法)/.test(skillType) ? '特殊' : skillType,
                属性: school || undefined,
                描述: effect || undefined,
                效果: effect || undefined,
                范围: range || undefined,
                射程: range || undefined,
                冷却: duration || undefined,
                备注: note || undefined
            });
            return;
        }
        ...
        nextSkills.push({
            id: skillId || `SKL_${index + 1}`,
            名称: name,
            类别: normalizeSkillCategory(skillType),
            描述: effect || undefined,
            效果: effect || undefined,
            触发: castingTime || undefined,
            持续: duration || undefined,
            范围: range || undefined,
            等级: tier || undefined,
            备注: note || undefined
        });
    ...
    state.角色.技能 = nextSkills as any;
    state.角色.魔法 = nextMagic as any;
};
...
const applyCharacterRegistryFromSheetRows = (state: GameState, rows: Array<Record<string, unknown>>) => {
    ...
    const name = toText(pickRowValue(target, ['姓名', 'name']));
    if (name) state.角色.姓名 = name;
    const occupation = toText(pickRowValue(target, ['职业', '称号', 'class', 'occupation']));
    if (occupation) state.角色.称号 = occupation;
    const appearance = toText(pickRowValue(target, ['外貌描述', '外貌', 'appearance']));
    if (appearance) state.角色.外貌 = appearance;
    const background = toText(pickRowValue(target, ['背景故事', '背景', 'background']));
    if (background) state.角色.背景 = background;

    const raceGenderAge = toText(pickRowValue(target, ['种族/性别/年龄', 'race_gender_age', '种族性别年龄']));
    if (raceGenderAge) {
    ...
    const familiaName = normalizeFamiliaName(
        pickRowValue(target, ['所属眷族', '眷族', '势力名称', '势力', 'familia', 'familiaName', 'faction'])
    );
    if (familiaName) {
        state.角色.所属眷族 = familiaName;
        state.角色.眷族 = familiaName;
        if (state.眷族 && familiaName !== '无') {
            state.眷族.名称 = familiaName;
        }
    } else if (state.角色.所属眷族 && !state.角色.眷族) {
        state.角色.眷族 = state.角色.所属眷族;
    }
};
...
export function handleUpsertSheetRows(
    ...
    switch (payload.sheetId) {
        ...
        case 'ECON_Ledger': {
            ...
            const syncResult = applyEconomicLedgerRowsToAccounts(state, normalizedRows);
        ...
        case 'SKILL_Library':
            shadowRows = normalizeSkillLibraryRows(payload.rows);
            keyField = 'SKILL_ID';
            break;
        case 'CHARACTER_Skills':
            shadowRows = normalizeCharacterSkillLinkRows(payload.rows);
            keyField = 'LINK_ID';
            break;
        ...
        case 'NPC_LocationTrace':
            ...
            shadowRows = payload.rows.filter((row) => {
                const playerName = state.角色?.姓名;
                const npcId = toText((row as any)?.npc_id ?? (row as any)?.NPC_ID ?? (row as any)?.id);
                const npcName = toText((row as any)?.npc_name ?? (row as any)?.NPC ?? (row as any)?.姓名);
                return !isPlayerReference(npcId, playerName) && !isPlayerReference(npcName, playerName);
            });
            break;
    ...
    if (payload.sheetId === 'SKILL_Library' || payload.sheetId === 'CHARACTER_Skills') {
        rebuildCharacterSkillsFromShadowStore(state);
    } else if (payload.sheetId === 'WORLD_NpcTracking') {
