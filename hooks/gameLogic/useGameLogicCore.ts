...
import { filterCommandsForServiceWithDiagnostics } from './microservice/commandGuard';
...
import { buildEquippedKeySet, isInventoryItemEquipped, reconcileEquipmentNameByInventory, syncInventoryEquippedByEquipment } from '../../utils/equipmentLinking';
...
import { consumeStateVariableEvents } from './microservice/stateVariableWriter';
import {
    mapLegacyPathToStateVariableTarget,
    STATE_VARIABLE_PILOT_SHEETS,
    type StateVariablePilotSheetId
} from '../../utils/taverndb/stateVariableMapping';
...
import { extractNpcPresenceEvidenceFromStateServiceInput, guardNpcPresenceCommands } from '../../utils/state/npcPresenceGuard';
...
const MEMORY_FILL_MAX_RETRIES = 2;
...
// 本文件里有多个 toNumberOrNull 变体，这里提供一个局部版本供“时间兜底”使用，避免 const 声明顺序导致的不可见问题。
const coerceNumberOrNull = (raw: unknown): number | null => {
...
const inferImplicitActionAdvanceMinutes = (
    input: string,
    ...
    // 玩家明确要求不推进时间时，尊重输入。
    if (/不推进时间|时间不变|暂停时间|冻结时间/.test(mergedText)) {
        return 0;
    }

    const hasRestSignal = /睡觉|睡下|过夜|次日|翌日|休整|长休|宿营|扎营|休息几个小时/.test(mergedText);
    const hasTravelSignal = /前往|赶路|出发|抵达|返回|回到|移动到|探索|调查|搜寻|巡逻|训练|锻造|制作|仪式|购物|采购|交易/.test(mergedText);
    const hasDialogueSignal = /聊天|交谈|对话|询问|寒暄|商议|讨论|请教|问路/.test(mergedText);
    const hasQuickDialogueSignal = /问了?两句|聊两句|随口问|简单聊|打个招呼|寒暄几句|确认一下|问个路|问路/.test(mergedText);
    const hasCombatSignal = /combat_|set_encounter_rows|upsert_battle_map_rows|resolve_attack_check|resolve_damage_roll|set_initiative|战斗|攻击|施法/.test(lower)
        || (Array.isArray(commands) && commands.some((cmd: any) => {
            const action = String(cmd?.action || '').trim().toLowerCase();
            const sheetId = String(cmd?.value?.sheetId || '').trim();
            return action.includes('combat') || action.includes('initiative') || sheetId.startsWith('COMBAT_');
        }));
    const movedLocation = !isSameLocation(prevState?.当前地点, nextState?.当前地点);

    // 分档时间范围约束，防止短互动被命令量“放大”为 1 小时以上。
    let baseMinutes = 10;
    let minMinutes = 5;
    let maxMinutes = 35;
    let commandWeight = 2;
    ...
    if (hasRestSignal) {
        ...
        commandWeight = 1;
        textWeight = 1;
    } else if (hasCombatSignal) {
        baseMinutes = 10;
        minMinutes = 5;
        maxMinutes = 25;
        commandWeight = 2;
        textWeight = 1;
    } else if (hasTravelSignal || movedLocation) {
        baseMinutes = movedLocation ? 30 : 24;
        minMinutes = 15;
        maxMinutes = movedLocation ? 90 : 60;
        commandWeight = 2;
        textWeight = 2;
    } else if (hasQuickDialogueSignal) {
        baseMinutes = 6;
        minMinutes = 2;
        maxMinutes = 12;
        commandWeight = 1;
        textWeight = 1;
    } else if (hasDialogueSignal) {
        baseMinutes = 10;
        minMinutes = 4;
        maxMinutes = 18;
        commandWeight = 1;
    ...
    const commandFactor = Math.min(10, Math.floor(commandCount / 4) * commandWeight);
    const textFactor = Math.min(8, Math.floor(mergedText.length / 260) * textWeight);
    let minutes = baseMinutes + commandFactor + textFactor;

    // 纯短对话兜底上限：未移动、非战斗、非长时行为时，不允许超过 12 分钟。
    if (hasQuickDialogueSignal && !movedLocation && !hasCombatSignal && !hasRestSignal && !hasTravelSignal) {
        maxMinutes = Math.min(maxMinutes, 12);
    }

    return clamp(Math.round(minutes), minMinutes, maxMinutes);
};

const ensureImplicitGlobalStateTimeAdvance = (
    prevState: GameState,
    nextState: GameState,
    contextType: 'ACTION' | 'PHONE' | 'WORLD' | 'MOMENT' | 'FORUM_POST' | 'FORUM_REPLY',
    input: string,
    logs: Array<{ sender?: string; text?: string }>,
    commands: TavernCommand[]
): GameState => {
    if (contextType !== 'ACTION') return nextState;
    const prevTime = String(prevState?.游戏时间 || '').trim();
    const nextTime = String(nextState?.游戏时间 || '').trim();
    const prevDate = String(prevState?.当前日期 || '').trim();
    const nextDate = String(nextState?.当前日期 || '').trim();
    if (!prevTime || !nextTime) return nextState;
    // 若本轮命令已显式推进时间/日期，则不做兜底。
    if (prevTime !== nextTime || prevDate !== nextDate) return nextState;

    const inferredMinutes = inferImplicitActionAdvanceMinutes(input, logs, commands, prevState, nextState);
    if (inferredMinutes <= 0) return nextState;

    const advanced = advanceGameTimeByMinutes(nextTime, inferredMinutes);
    const advancedDate = advanceDateString(nextDate || prevDate, advanced.dayDelta);
    const coordX = coerceNumberOrNull((nextState as any)?.世界坐标?.x);
    const coordY = coerceNumberOrNull((nextState as any)?.世界坐标?.y);
    const row: Record<string, unknown> = {
        _global_id: GLOBAL_STATE_ROW_ID,
        当前回合: Math.max(0, Math.floor(Number(nextState?.回合数 || prevState?.回合数 || 0))),
        当前场景: String(nextState?.当前地点 || '').trim() || undefined,
        场景描述: String((nextState as any)?.场景描述 || '').trim() || undefined,
        当前日期: advancedDate || undefined,
        游戏时间: advanced.time || undefined,
        上轮时间: nextTime || undefined,
        流逝时长: formatElapsedDuration(inferredMinutes),
        世界坐标X: coordX ?? undefined,
        世界坐标Y: coordY ?? undefined,
    ...
    const result = handleUpsertSheetRows(nextState, {
        sheetId: 'SYS_GlobalState',
        keyField: '_global_id',
        rows: [row]
    });
    if (!result.success) {
        // 兜底失败时至少保持 runtime 时间推进，避免回合时间卡死。
        (nextState as any).上轮时间 = nextTime;
        (nextState as any).游戏时间 = advanced.time;
        (nextState as any).当前日期 = advancedDate;
        (nextState as any).流逝时长 = formatElapsedDuration(inferredMinutes);
    }
    return nextState;
};
...
const reconcileEquipmentWithInventory = (state: GameState): GameState => {
    ...
    if ((!equipment || typeof equipment !== 'object') && inventory.length === 0) return state;

    const normalizedSlots: Record<string, string> = {};
    Object.entries((equipment && typeof equipment === 'object') ? equipment : {}).forEach(([slot, raw]) => {
    ...
    const baseEquipment = (equipment && typeof equipment === 'object') ? equipment : {};
    const nextEquipment = {
        ...baseEquipment,
        ...reconciled.equipment
    };
    const inventorySync = syncInventoryEquippedByEquipment(inventory, nextEquipment);
    if (!reconciled.changed && !inventorySync.changed) return state;

    return {
        ...state,
        ...(inventorySync.changed ? { 背包: inventorySync.inventory } : {}),
        角色: {
            ...state.角色,
            装备: nextEquipment
        }
...
const buildInventorySyncRows = (state: GameState): Array<Record<string, unknown>> => {
    ...
    return inventory
        .map((item, index) => {
            ...
            const qualityRaw = String((item as any).品质 ?? (item as any).稀有度 ?? '').trim();
            const normalizedQuality = qualityRaw ? normalizeQualityLabel(qualityRaw) : undefined;
            return {
                物品ID: itemId,
                物品名称: name,
...
const applyDurabilityWear = (state: GameState, action: DurabilityAction): GameState => {
    ...
    const nextInventory = inventory.map(item => {
        ...
        const baseQualityRaw = String(item.品质 ?? item.稀有度 ?? '').trim();
        const nextQuality = nextDurability === 0
            ? '破损'
            : (baseQualityRaw ? normalizeQualityLabel(baseQualityRaw) : undefined);
        return { ...item, 耐久: nextDurability, 品质: nextQuality, 稀有度: nextQuality };
    });
...
export const useGameLogic = (initialState?: GameState, onExitCb?: () => void) => {
    ...
    const processTavernCommands = (state: GameState, commands: TavernCommand[]): { newState: GameState, logs: LogEntry[], hasError: boolean, sheetPatches?: TavernDBSheetPatch[] } => {
        ...
        const expectedSheetVersionCursor = new Map<string, number>();
        const buildSheetPatchesForCommand = (action: string, cmd: TavernCommand): TavernDBSheetPatch[] => {
            ...
            const nextExpectedSheetVersion = (sheetId: TavernDBSheetId): number | undefined => {
                ...
                const lastAssigned = expectedSheetVersionCursor.get(key);
                const next = typeof lastAssigned === 'number'
                    ? (base > lastAssigned ? base : lastAssigned + 1)
                    : base;
                expectedSheetVersionCursor.set(key, next);
                return next;
            };
        ...
        const rewriteLegacyBusinessCommand = (
                ...
                if (action === 'set' || action === 'push') {
                    ...
                    const rows = sourceRows
                        .map((item) => {
                            ...
                            const itemId = String(row.id ?? row.物品ID ?? name).trim() || undefined;
                            const qualityRaw = String(row.品质 ?? row.稀有度 ?? row.quality ?? row.rarity ?? row.rank ?? row.tier ?? '').trim();
                            const quality = qualityRaw ? normalizeQualityLabel(qualityRaw) : undefined;
                            const type = String(row.类型 ?? row.类别 ?? row.type ?? '').trim() || undefined;
                            return {
                                ...row,
                                id: itemId,
                                物品ID: itemId,
            ...
            if (inventoryMatch) {
                ...
                const baseItemId = String((targetItem as any)?.id ?? (targetItem as any)?.物品ID ?? '').trim() || undefined;
                const baseName = String((targetItem as any)?.名称 ?? (targetItem as any)?.物品名称 ?? '').trim();
                const baseType = String((targetItem as any)?.类型 ?? (targetItem as any)?.类别 ?? '').trim() || undefined;
                const baseQualityRaw = String((targetItem as any)?.品质 ?? (targetItem as any)?.稀有度 ?? '').trim();
                const baseQuality = baseQualityRaw ? normalizeQualityLabel(baseQualityRaw) : undefined;
                const row: Record<string, unknown> = {
                    id: baseItemId,
                    物品ID: baseItemId,
                    名称: baseName,
                ...
                if (!rawField && action === 'set' && value && typeof value === 'object' && !Array.isArray(value)) {
                    ...
                    const nextItemId = String(merged.id ?? merged.物品ID ?? row.id ?? name).trim() || undefined;
                    const nextType = String(merged.类型 ?? merged.类别 ?? merged.type ?? row.类型 ?? '').trim() || undefined;
                    const qualityRaw = String(merged.品质 ?? merged.稀有度 ?? merged.quality ?? merged.rarity ?? merged.rank ?? merged.tier ?? row.品质 ?? '').trim();
                    const quality = qualityRaw ? normalizeQualityLabel(qualityRaw) : undefined;
                    return {
                        action: 'upsert_sheet_rows',
                        value: {
                ...
                if (field === '数量' || field === '价值') {
                    ...
                    const type = String(value ?? '').trim() || undefined;
                    row.类型 = type;
                    row.类别 = type;
                } else if (['名称', '描述'].includes(field)) {
                    const text = String(value ?? '').trim();
                    row[field] = text;
                    if (field === '名称') row.物品名称 = text;
                } else {
                    return null;
                }
        ...
        // Stage B bridge: 从 runtime 命令生成变量事件，先用于审计/观测，不接管现有落表路径。
        const bridgedStateVariableEvents = buildStateVariableEventsFromCommands(commandBatch, {
            turnId: String(Math.max(0, Math.floor(Number(nextState.回合数 || state.回合数 || 0)))),
            source: 'runtime:state-variable-bridge',
            includeSheets: ['SYS_GlobalState', 'CHARACTER_Resources', 'ITEM_Inventory']
        });
        if (bridgedStateVariableEvents.length > 0) {
            (nextState as any).__stateVarBridge = {
                turnId: String(nextState.回合数 || state.回合数 || 0),
                events: bridgedStateVariableEvents
            };
            const stateVarWriterSettings = settings.stateVarWriter || {
                enabled: false,
                shadowMode: false,
                cutoverDomains: [],
                rejectNonWriterForCutoverDomains: false
            };
            if (stateVarWriterSettings.enabled || stateVarWriterSettings.shadowMode) {
                const writerShadowResult = consumeStateVariableEvents({
                    stateSnapshot: nextState,
                    events: bridgedStateVariableEvents,
                    shadowMode: true
                });
                (nextState as any).__stateVarWriterShadow = {
                    eventCount: writerShadowResult.acceptedEvents.length,
                    skippedCount: writerShadowResult.skippedEvents.length,
                    commandCount: writerShadowResult.commands.length,
                    auditCommandCount: writerShadowResult.auditCommands.length,
                    updatedAt: Date.now()
                };
            }
        }
    ...
    const CUTOVER_DOMAIN_TO_SHEET: Record<string, StateVariablePilotSheetId> = {
        global_state: 'SYS_GlobalState',
        character_resources: 'CHARACTER_Resources',
        inventory: 'ITEM_Inventory'
    };
    const resolveStateWriterCutoverSheets = (): StateVariablePilotSheetId[] => {
        const writerSettings = settings.stateVarWriter || {
            enabled: false,
            shadowMode: false,
            cutoverDomains: [],
            rejectNonWriterForCutoverDomains: false
        };
        if (!writerSettings.enabled) return [];
        const rawDomains = Array.isArray(writerSettings.cutoverDomains) ? writerSettings.cutoverDomains : [];
        const sheetSet = new Set<StateVariablePilotSheetId>();
        rawDomains.forEach((rawDomain) => {
            const normalized = String(rawDomain || '').trim();
            if (!normalized) return;
            if ((STATE_VARIABLE_PILOT_SHEETS as readonly string[]).includes(normalized)) {
                sheetSet.add(normalized as StateVariablePilotSheetId);
                return;
            }
            const mapped = CUTOVER_DOMAIN_TO_SHEET[normalized];
            if (mapped) sheetSet.add(mapped);
        });
        return Array.from(sheetSet);
    };
    const resolveCommandSheetId = (cmd: TavernCommand): string => {
        const action = String((cmd as any)?.action ?? (cmd as any)?.type ?? '').trim();
        if (action !== 'upsert_sheet_rows' && action !== 'delete_sheet_rows') return '';
        const value = (cmd as any)?.value;
        if (Array.isArray(value)) {
            const candidate = value.find((item) => item && typeof item === 'object' && !Array.isArray(item)) as any;
            return String(candidate?.sheetId ?? candidate?.sheet_id ?? '').trim();
        }
        return String(
            (value as any)?.sheetId
            ?? (value as any)?.sheet_id
            ?? (cmd as any)?.sheetId
            ?? (cmd as any)?.sheet_id
            ?? ''
        ).trim();
    };
    const commandTouchesCutoverSheet = (
        cmd: TavernCommand,
        cutoverSheetSet: Set<StateVariablePilotSheetId>
    ): boolean => {
        const sheetId = resolveCommandSheetId(cmd);
        if (sheetId && cutoverSheetSet.has(sheetId as StateVariablePilotSheetId)) return true;
        const action = String((cmd as any)?.action ?? (cmd as any)?.type ?? '').trim();
        if (action !== 'set' && action !== 'add' && action !== 'push' && action !== 'delete') return false;
        const target = mapLegacyPathToStateVariableTarget((cmd as any)?.key ?? (cmd as any)?.path);
        if (!target) return false;
        return cutoverSheetSet.has(target.sheetId);
    };
    const rewriteCommandsForStateWriterCutover = (
        stateSnapshot: GameState,
        commands: TavernCommand[]
    ): { stateSnapshot: GameState; commands: TavernCommand[] } => {
        const cutoverSheets = resolveStateWriterCutoverSheets();
        if (cutoverSheets.length === 0 || !Array.isArray(commands) || commands.length === 0) {
            return { stateSnapshot, commands };
        }
        const cutoverSheetSet = new Set<StateVariablePilotSheetId>(cutoverSheets);
        const cutoverEvents = buildStateVariableEventsFromCommands(commands, {
            turnId: String(Math.max(0, Math.floor(Number(stateSnapshot?.回合数 || 0)))),
            source: 'runtime:state-variable-cutover',
            includeSheets: cutoverSheets
        });
        if (cutoverEvents.length === 0) {
            return { stateSnapshot, commands };
        }
        const writerConverted = consumeStateVariableEvents({
            stateSnapshot,
            events: cutoverEvents,
            shadowMode: true
        });
    ...
    const applyCommandsWithTurnTransactionBase = createApplyCommandsWithTurnTransaction(
        processTavernCommands,
        () => {
            const writerSettings = settings.stateVarWriter || {
                enabled: false,
                shadowMode: false,
            ...
            const cutoverSheets = resolveStateWriterCutoverSheets();
            if (!writerSettings.rejectNonWriterForCutoverDomains || cutoverSheets.length === 0) return {};
            return {
                sourceOwnershipRules: {
                    protectedSheets: cutoverSheets,
                    allowedSourcePrefixes: ['ms:state-writer']
                }
            };
        }
    );
    const applyCommandsWithTurnTransaction = (stateSnapshot: GameState, commands: TavernCommand[]) => {
        const rewritten = rewriteCommandsForStateWriterCutover(stateSnapshot, commands);
        return applyCommandsWithTurnTransactionBase(rewritten.stateSnapshot, rewritten.commands);
    };
    ...
    const enqueueMicroserviceTask = (
        ...
        stateSnapshot: GameState,
        priority: number,
        taskOptions: { memoryJobs?: MemoryFillJob[] } = {}
    ) => {
        ...
        const taskMemoryJobs = serviceKey === 'memory' && Array.isArray(taskOptions.memoryJobs)
            ? taskOptions.memoryJobs
            : [];
        microserviceQueueRef.current.enqueue(async () => {
            ...
            try {
                ...
                if (!result.tavern_commands || result.tavern_commands.length === 0) {
                    ...
                    if (serviceKey === 'memory' && taskMemoryJobs.length > 0) {
                        retryMemoryFillJobs(taskMemoryJobs, 'empty');
                    }
                    return;
                }
                ...
                const guardResult = filterCommandsForServiceWithDiagnostics({
                    ...
                    strictAllowlist: serviceKey === 'state'
                        ? settings.stateVarWriter?.governance?.domainScope?.strictAllowlist !== false
                        : false,
                ...
                if (guardRejected.length > 0 && shouldDebugMicroservice()) {
                    console.warn('[DXC][MS GUARD]', serviceKey, {
                        rejected: guardRejected.slice(0, 12),
                        rejectedCount: guardRejected.length
                    });
                }
                ...
                if (validCommands.length === 0) {
                    ...
                    if (serviceKey === 'memory' && taskMemoryJobs.length > 0) {
                        retryMemoryFillJobs(taskMemoryJobs, 'filtered-empty');
                    }
                    return;
                }
                if (serviceKey === 'memory' && taskMemoryJobs.length > 0) {
                    const expectedTurns = new Set(
                        taskMemoryJobs
                            .map((job) => Math.max(1, Math.floor(Number(job?.turnIndex || 0))))
                            .filter((turn) => Number.isFinite(turn))
                    );
                    const normalizeMemoryTurn = (cmd: TavernCommand, sheet: MemoryTargetSheet): number | null => {
                        const turn = extractMemoryCommandTurn(cmd, sheet);
                        if (!Number.isFinite(turn) || turn <= 0 || turn >= Number.MAX_SAFE_INTEGER) return null;
                        return Math.floor(turn);
                    };
                    const summaryTurns = new Set<number>();
                    const outlineTurns = new Set<number>();
                    validCommands.forEach((cmd) => {
                        if (filterMemoryCommandsBySheet([cmd], 'LOG_Summary').length > 0) {
                            const turn = normalizeMemoryTurn(cmd, 'LOG_Summary');
                            if (turn !== null) summaryTurns.add(turn);
                        }
                ...
                let npcPresenceGuardBlocked = 0;
                if (serviceKey === 'state') {
                    const evidence = extractNpcPresenceEvidenceFromStateServiceInput(input, stateSnapshot);
                    if (evidence.presentNpcNames.size > 0) {
                        const guarded = guardNpcPresenceCommands(sanitizedCommands, evidence);
                        sanitizedCommands = guarded.commands;
                        npcPresenceGuardBlocked = guarded.blocked;
                    }
                }
                ...
                if (npcPresenceGuardBlocked > 0) {
                    pushDebugToast('MS', `${serviceKey} npc-guard=${npcPresenceGuardBlocked}`);
                }
                ...
                setGameState(prev => {
                    ...
                    if (visibleRejected.length > 0) {
                        ...
                        const reasonSummary = Object.entries(byReason)
                            ...
                            .join(', ');
                        const hintSummary = topHints.length > 0 ? ` sample=${topHints.join(' | ')}` : '';
                        next.日志 = [
                            ...(next.日志 || []),
                            {
                                id: generateLegacyId(),
                    ...
                    if (serviceKey === 'state') {
                        const softWarn = settings.stateVarWriter?.governance?.turnScope?.crossTurnSoftWarning;
                        const crossTurnWarning = evaluateCrossTurnSoftWarning(
                            guardedCommands,
                            Math.max(1, Math.floor(Number(prev.回合数 || 0))),
                            softWarn
                        );
                        if (crossTurnWarning.count > 0) {
                            next.日志 = [
                                ...(next.日志 || []),
                                {
                                    id: generateLegacyId(),
                                    sender: '系统',
                                    text: `[MS_TURN_WARN_001] 检测到跨回合写入 soft-warning count=${crossTurnWarning.count}, threshold=${crossTurnWarning.threshold}, sampling=${crossTurnWarning.sampling}${crossTurnWarning.samples.length > 0 ? ` sample=${crossTurnWarning.samples.join(' | ')}` : ''}`,
                                    timestamp: Date.now(),
                                    type: 'system'
                                } as any
                            ];
                        }
                        const rollbackSummary = ((next as any)?.__tableMeta?.stateVarOutcome?.rollback && typeof (next as any).__tableMeta.stateVarOutcome.rollback === 'object')
                            ? (next as any).__tableMeta.stateVarOutcome.rollback
                            : {};
                        const writerMetrics = ((next as any)?.__stateVarWriter?.metrics && typeof (next as any).__stateVarWriter.metrics === 'object')
                            ? (next as any).__stateVarWriter.metrics
                            : {};
                        const writerSkipByReason = (writerMetrics.skipByReason && typeof writerMetrics.skipByReason === 'object')
                        ...
                        (next as any).__stateVarDiagnostics = {
                            ...
                            lastStateRun: {
                                serviceKey,
                                timestamp: Date.now(),
                                rollbackReason: String(rollbackSummary.reason || ''),
                                writerSkipped,
                                writerSkipByReason: {
                                    duplicate_idempotency: Math.max(0, Math.floor(Number(writerSkipByReason.duplicate_idempotency || 0))),
                                    stale_event: Math.max(0, Math.floor(Number(writerSkipByReason.stale_event || 0))),
                                    invalid_event: Math.max(0, Math.floor(Number(writerSkipByReason.invalid_event || 0))),
                ...
                if (isAbortError(e)) {
                    ...
                    if (serviceKey === 'memory' && taskMemoryJobs.length > 0 && taskEpoch === stateEpochRef.current) {
                        retryMemoryFillJobs(taskMemoryJobs, 'aborted');
                    }
                    return;
                }
                ...
                if (serviceKey === 'memory' && taskMemoryJobs.length > 0) {
                    retryMemoryFillJobs(taskMemoryJobs, `error:${message}`);
                }
    ...
    const flushMemoryFillJobs = () => {
        ...
        for (let start = 0; start < ordered.length; start += batchSize) {
            ...
            enqueueMicroserviceTask(
                'memory',
                memoryInput,
                latestState,
                MICROSERVICE_PRIORITIES.memory,
                { memoryJobs: batch }
            );
            pushDebugToast(
                'MS MEM',
                `batch=${batch.length} turns=${batch.map((j) => j.turnIndex).join(',')} retries=${batch.map((j) => j.retryCount || 0).join(',')}`
            );
        }
    };
    ...
    const retryMemoryFillJobs = (
        ...
        const retriable = jobs
            ...
            .filter((job) => (job.retryCount || 0) <= MEMORY_FILL_MAX_RETRIES);
        if (retriable.length === 0) {
            pushDebugToast('MS MEM', `retry-drop reason=${reason} exhausted=Y`);
            return;
        }
        memoryFillJobsRef.current.push(...retriable);
        const maxAttempt = Math.max(...retriable.map((job) => job.retryCount || 0));
        pushDebugToast(
            'MS MEM',
            `retry reason=${reason} jobs=${retriable.length} turns=${retriable.map((job) => job.turnIndex).join(',')} attempt=${maxAttempt}`
        );
    ...
    const handleAIInteraction = async (
        ...
        try {
            ...
            newState = ensureImplicitGlobalStateTimeAdvance(
                stateWithUserLog,
                newState,
                contextType,
                input,
    ...
    const applyAiResponseToState = (
        ...
        newState = ensureImplicitGlobalStateTimeAdvance(
            ...
            'ACTION',
            '',
            logsForResponse.map((log) => ({ sender: String(log?.sender || ''), text: String(log?.text || '') })),
            commands
        );
        if ((state.回合数 || 1) <= 1) {
