...
export const buildServicePrompt = (serviceKey: string, input: string, gameState: GameState, settings: AppSettings): string => {
    ...
    const memoryFillRule = serviceKey === 'memory'
        ? [
            '【Memory Fill Rules】',
            '1. 你的任务是填写 LOG_Summary 与 LOG_Outline。',
            '2. 若 [服务输入] 含 待填回合 数组，则按目标回合升序逐个处理。',
            '3. [并发与配对约束] 若 [服务输入].填表任务.targetSheet 存在，本次【只允许】输出该表对应命令，严禁跨表和成对输出！',
            '4. 若 targetSheet 不存在，则每个回合必须【成对】生成 append_log_summary 和 append_log_outline。',
            '5. 在成对写入模式下，两条命令必须共享同一 编码索引（AMxxxx）；若独立模式则可省略，由运行时自动分配。',
            '6. 时间格式硬约束：时间=YYYY-MM-DD HH:MM；时间跨度=YYYY-MM-DD HH:MM—YYYY-MM-DD HH:MM。',
            '7. LOG_Summary（总结表）字段语义：时间跨度 / 地点 / 纪要 / 重要对话 / 编码索引。',
            '8. LOG_Outline（总体大纲）字段语义：时间跨度 / 大纲 / 编码索引。',
            '9. 总结表纪要必须是“高保真、零解读”的客观记录：仅写可见动作与可闻信息，禁止心理、动机、意义升华；字数硬约束 180-240，目标约 200 字。',
            '10. 总结表重要对话只摘录关键原句（需标注说话者），总长度不超过80 token。',
            '11. 总体大纲必须是对同回合总结表的“精炼主干”，建议 40-120 字，只保留核心事件链。',
            '12. 反同质化硬约束：大纲与纪要禁止高度近似（同句改写或大量重合），若同质化必须重写大纲。',
            '13. append_log_summary.value 至少包含模板最小字段: 时间跨度, 地点, 纪要(或摘要), 重要对话(可空)。',
            '14. append_log_outline.value 至少包含模板最小字段: 时间跨度, 大纲。',
            '15. 已存在记录跳过原则：仅针对本次 targetSheet 检查是否已存在记录；如果已存在则跳过；全部已存在时返回 {"tavern_commands": []}。',
            '16. 输出前自检：确认是否出现多余表的命令（严禁在targetSheet单表模式下输出另一张表），检查是否全返回空命令；只有当无须更新时才返回空。',
            '17. 禁止输出任何非日志表命令。'
        ].join('\n')
        : '';
