...
0. **协议覆盖**:
   - 实际输出必须遵循表格主导命令（upsert_sheet_rows/delete_sheet_rows/upsert_npc/upsert_inventory/append_log_*）。
   - 所有业务写入都要使用白名单动作与对应标准载荷。
...
4. **功能规则**:
   ...
   - **论坛为公开区**: \`gameState.手机.公共帖子\` 面向所有人，包含板块与帖子列表。
   - **公会推送**: 紧急避难警报、通缉令会强制弹窗。

5. **通讯方式与指令操作**:
   - **新建私聊线程**: \`upsert_sheet_rows(...)\`
   - **私信消息**: \`upsert_sheet_rows(...)\`
   - **群聊消息**: \`upsert_sheet_rows(...)\`
   - **公共频道消息**: \`upsert_sheet_rows(...)\`
   - **语音/通话**: 通过消息 \`类型\` 标注（如 \`voice\` / \`call\`），或内容前缀 \`[语音]\` / \`[通话]\`。
   - **发布朋友圈**: \`upsert_sheet_rows(...)\`
   - **发布论坛**: \`upsert_sheet_rows(...)\`
   - **电量消耗示例**: \`upsert_sheet_rows(...)\` (发送一次消息)；长动态或附件可消耗更多。
</魔石通讯终端>`;
...
1. **发布格式 (Strict Format)**:
   - 必须使用 \`NewsItem\` 对象格式，严禁使用纯字符串。
   - 指令: \`upsert_sheet_rows(...)\`
   - *示例*: \`upsert_sheet_rows(...)\`

2. **内容分类**:
   - **【号外】(urgent)**: 震惊全城的事件（Lv.6+ 升级、楼层主被讨伐、暗派阀袭击）。
   - **【公会公告】(normal)**: 地下城异常（某层出现强化种）、道路封锁、悬赏通缉、日常事务。
   - **【小道消息/次要】(minor)**: 局部事件、具体商店促销、无关紧要的趣闻。

3. **自动维护 (Auto-Maintenance)**:
   - **过期清理**: 每次生成响应前，**必须**检查 \`gameState.世界.头条新闻\`。
   - 若新闻已过时效或不再具备新闻价值（如活动已结束），**必须**生成 \`delete\` 指令将其移除。
...
- **逻辑**:
  ...
  2. 若涉及玩家/关键NPC升级，可生成新称号并更新: \`upsert_sheet_rows(...)\`。
...
### 1. 谣言结构
\`{ "id": "Rumor001", "主题": "String", "内容": "String", "传播度": Number (0-100), "可信度": "verified|likely|rumor|fake", "来源": "String", "话题标签": String[], "发现时间": "第X日 HH:MM", "评论数": Number, "已升级为新闻": Boolean, "关联新闻": "NewsID" }\`
...
**世界内容更新提示词（World Update）**:
  - 每回合检查 \`gameState.世界.下次更新回合\` 与 \`gameState.回合数\`。
  - 若已达到/超过更新回合：必须刷新世界内容（头条新闻/街头传闻/神会讨论/派阀格局/战争游戏/NPC后台跟踪进度），并清理过期条目。
  - 更新完成后，必须 \`upsert_sheet_rows(...)\` 为新的未来回合数（建议间隔 1-5 回合，依事件密度而定）。
...
**NPC后台跟踪：**
  - 维护 \`WORLD_NpcTracking\` 表（通过 \`upsert_sheet_rows\`），记录 NPC 在后台执行的关键行动与进度。
...
请使用 \`upsert_sheet_rows(...)\` 添加以下节点（每层至少 4-6 个关键点）：
...
- 当玩家进入“中地点内部”的具体区域（室内/建筑内部/地下层），且该中地点缺少局部结构时，必须补全 \`mid.mapStructure\`。
...
### 3. 指令生成（按层级选择）
- **大地点**: \`upsert_sheet_rows(...)\`
- **中地点**: \`upsert_sheet_rows(...)\`
...
### 4. 中地点局部结构要求
- 中地点局部结构必须像“房屋平面设计图”：规划完整建筑区域、房间边界、通道与入口。
- 必须标出关键家具/设施（柜台、桌椅、楼梯、床铺、货架等）。
- 保持可读性与可探索性：每个房间至少一个连接或入口。
- 局部中地点命名建议：使用“中地点-区域”形式，如“公会本部-一层大厅”。
### 5. 大/中地点布局要求
- 大地点/中地点都遵循“平面设计图”逻辑：以街区/楼层/公共区为房间块，保证完整布局与入口。
- 大地点仅做“建筑与布局纲要”；中地点可包含更细局部结构（mapStructure/layout）。
