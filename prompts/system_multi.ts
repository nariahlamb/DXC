...
export const P_SYS_FORMAT_MULTI = (() => {
  ...
  output = replaceOnce(
    ...
    '**输出顺序必须是**：thinking_plan → thinking_style → thinking_draft → thinking_check → thinking_canon → thinking_vars_pre → thinking_vars_other → thinking_vars_merge → thinking_gap → logs → thinking_vars_post → tavern_commands → shortTerm'
  );
