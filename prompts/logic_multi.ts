...
export const P_COT_LOGIC_MULTI = (() => {
  ...
  output = output.replace(
    ...
    [
      '## 输出分段要求',
      '- 剧情预先思考（thinking_plan）：思考剧情变量：gameState.剧情相关内容，依照剧情规划进行初步思考，判断是否需要修订剧情变量。',
      '- 文风思考（thinking_style）：确定本回合的视角、语气、节奏与感官重点。',
      '- 剧情草稿（thinking_draft）：输出剧情大致走向草稿（允许叙事文本，不写指令）。',
      '- 剧情合理性校验（thinking_check）：基于草稿与上下文做合理性检查。',
      '- 原著思考（thinking_canon）：检查角色/世界观一致性/名称校验是否正确。',
