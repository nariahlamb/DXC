...
4. **楼层生态 (Floor Ecology)**:
   - **上层 (1-12F)**: 入门区，地形相对稳定，适合 Lv.1 新人试炼。
   - **中层 (13-24F)**: “死线”之后，地形复杂、怪物更具战术性。
   - **安全层**: **18F(Under Resort)**、**28F(Under Garden)**、**39F(Under Bridge)**、**50F** 为安全层，城镇或补给区内不生成普通怪物。
   - **下层 (25-36F)**: “水之迷都”等大型生态，环境极端，怪物强度显著提升。
   - **深层 (37-50F)**: 生态稳定但危险指数陡升，被视为人类禁区，仅少数队伍能深入。
   - **楼层主示例**: 17F「歌利亚」、27F「安菲斯比纳」、37F「乌代俄斯」、49F「巴罗尔」。
   - **探索门槛参考**: 1-12F 面向 Lv.1；13-24F 面向 Lv.2+；25-36F 面向 Lv.3+；37-50F 面向 Lv.4+ 主力眷族。
   - **50F以下更深层**: 资料稀少，作为“超深层/未知区域”处理，必须谨慎设定与逐层拓展。
...
2. **刷新节奏（建议）**:
   ...
   - 中层(13-24F)：5-15 分钟/波，单次 3-8 只。
   - 下层(25-36F)：3-10 分钟/波，单次 4-10 只。
   - 深层(37-50F)：1-5 分钟/波，单次 6-12 只，且更易出现强化种。
...
4. **安全层规则**:
   - 安全层（18/28/39/50F）的城镇与明确安全区内不刷新普通怪物。
   - 安全层“野外边缘”可有少量低威胁怪物，但强度必须低于相邻楼层。

5. **合理性校验**:
   - 刷怪强度必须匹配楼层危险度与玩家等级，禁止低等级在深层“无压力刷怪”。
   - 低等级进入深层**不是绝对禁止**，但必须有可信前提（护送/异常期/临时通路/强力支援/撤退路线），并体现极高风险与代价。
</地下城刷怪逻辑>`;
...
6. **NPC 能力值生成规则 (NPC Stat Generation)**:
   - 当 NPC 没有能力值/生存数值时，按“等级 + 天分 + 职业/种族”生成。
   - **一般人基准**:
     - 选定 1 个主属性作为“升级标准”，其 **隐藏基础能力 = 400 × Lv**。
     - 其余 4 项 **隐藏基础能力 = 主属性的 1/5**。
     - 该基准代表“普通人水平”，再据此调整 NPC 天分与职业倾向。
   - **天分档位** (基于一般人标准的倍率调整):
     - 低: 0.7×
     - 普通: 1.0×
     - 优秀: 1.2×
     - 顶尖: 1.4×
     - 怪物级: 1.7×
   - **分配权重**: 战士偏力量/耐久/敏捷，法师偏魔力/灵巧，支援偏灵巧/敏捷。
   - **派生数值**: 生成完成后必须按公式计算 HP/精神力/体力/负重，并写入 \`生存数值\`。
   - **禁止重算**: 若 NPC 已有能力值或生存数值，禁止二次重算。
</能力值与精神力>`;
...
4. **战斗数据维护 (UI/State)**:
   - 战斗进行中时，\`gameState.战斗.敌方\` 必须为数组（即使只有 1 个敌人）。
   - 敌方对象至少包含：\`id/名称/当前生命值/最大生命值/攻击力/描述/等级\`，精神力相关可选。
