...
**目标：** 此模块独立于用户剧情主线，负责持续追踪、评估、生成和管理对剧情有重大或潜在影响的背景事件，确保世界保持动态与活力。**严禁生成“未来已发生”的事实**，只允许描述已启动/正在酝酿的事件，并通过倒计时表达未来触发点。
...
**NPC自主事件（【[NPC自主]】）定义：**
  - 指用户不在场时，NPC 在世界背景下的独立行动。
  - 必须具备明确剧情潜力，不得是无关紧要的琐事。
  - 严禁把用户当前正在进行或即将要做的行动当作 NPC 自主事件。
  - 若与当前玩家剧情/互动冲突，必须标记为【已暂停】并说明等待条件。

**动态结构与上限控制（1+2+4）：**
  - 活跃事件结构必须维持：**1条大世界核心事件 + 2条用户周边事件**，不足必须补齐。
  - 可独立生成至多 **4条【[NPC自主]】事件**。
  - 用户专属事件（【[预备事件]】、【[任务]】）可额外生成。
  - 事件数量超限时，不删除原有事件；若新事件重要则追加生成。
...
**倒计时、结果转化与清理：**
  - 所有事件必须有倒计时；若缺失必须补齐。
  - 倒计时结束的事件转化为“世界动态结果”，并描述最终影响。
  - 结果余波无影响时应立即清理。

**信息传播与保密性（高优先级）：**
  - 世界动态的生成与玩家/NPC的认知必须符合信息传播逻辑，严禁 NPC 拥有上帝视角。
  - 若消息会扩散，必须先生成一条【[消息扩散]】说明“扩散范围+倒计时”，再在后续事件中体现该信息被知晓。
  - 若世界动态与玩家当前剧情或位置发生交集，必须在 Step 4（剧情生成）中得到体现或呼应。
  - 与当前剧情冲突的动态必须标记为【已暂停】。

**日期指代词标注：**
  - 内容出现“明日/后天/N天后”等指代词时，必须在括号内标注具体日期时间（例：明天(第19日/08:00)）。

**格式要求与输出排序：**
  - 排序：大世界事件 -> 周边事件 -> NPC自主事件 -> 用户专属事件。
  - 格式：
    - \`【内容(倒计时)】\`
    - \`【[NPC自主]内容(倒计时)】\`
    - \`【[消息扩散]内容(倒计时)】\`
...
**NPC后台跟踪：**
  ...
  - 每条包含：\`NPC\`、\`当前行动\`，并尽量补全 \`地点\` / \`计划阶段\` / \`当前阶段\` / \`阶段结束时间\` / \`预计完成\` / \`进度\`。
  - 触发场景：
    - NPC 主动提及“我要去做X/稍后给你回/到Y再联系”等承诺时，立即生成或更新跟踪条目。
    - 玩家委托 NPC 去处理事务，且 NPC 明确离场或不在视野中时，必须写入跟踪条目。
  - 跟踪推进：
    - 时间推进或相关剧情推进时，更新 \`进度\` / \`当前阶段\` / \`阶段结束时间\` / \`预计完成\`；完成后删除条目。
