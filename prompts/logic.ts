...
## 0.2 手机联动规划
- 若系统提示词明确启用“手机独立API”（包含相关规则），当剧情涉及**手机聊天/通知/论坛/联系人推进**时，在最终 JSON 顶层**新增** "phone_sync_plan" 字段，输出一句话规划（供手机 API 生成消息）。
...
- "phone_sync_plan" 输出为**一句话字符串**（类似 shortTerm），禁止输出 JSON 对象或数组。
- "phone_sync_plan" 必须是**独立的顶层字段**，单独成块输出；不得混入 thinking/logs/tavern_commands/shortTerm，也不得夹带注释或解释性文本。
- 若目标联系人当前在场（社交上下文显示是否在场=true），phone_sync_plan 必须避免“手机私聊当场对话”的冲突：改为延迟/触发条件，或改由系统/不在场联系人发消息。
- 允许基于 phone context 与剧情需要制定计划，不要求 logs 中出现具体消息内容。
- 手机消息/帖子更新优先通过 "phone_sync_plan" 触发手机系统生成，避免在 tavern_commands 中重复写入手机内容。
- phone_sync_plan 默认即时触达；如需等待/延时必须明确时间或触发条件，避免主剧情出现提醒而手机无消息。
...
- 构思具体变更内容（无痕化与精确化）：
  ...
  - 手机与动态：
    - 若启用手机独立API：手机相关内容通过 "phone_sync_plan" 交给手机系统生成，不在 tavern_commands 中写入。
    - 若未启用：仅在叙事发生时 push gameState.手机.对话/朋友圈/公共帖子。
